...
function () {
  ...
  function UnitConverter() {
    ...
    strict.equal(_typeof(this.inputMeasurementSystem), 'object');
    strict.equal(_typeof(this.outputMeasurementSystem), 'object');
    strict.ok(['ellipsoid', 'geoid'].indexOf(this.inputZRef) !== -1, 'inputZRef must be oneof "ellipsoid" or "geoid"');
    strict.ok(['ellipsoid', 'geoid'].indexOf(this.outputZRef) !== -1, 'outputZRef must be oneof "ellipsoid" or "geoid"'); // expose unit for testing
  ...
  _createClass(UnitConverter, [{
    ...
    value: function position(x, y, z) {
      ...
      strict.equal(!!matrix || !!parameters, true, 'To convert a position you must specify either a transform matrix or parameters'); // matrix if present takes precendence over parameters and geoidHeight
    ...
    value: function _applyParametersAndGeoidToPoint(x, y, z) {
      strict.equal(_typeof(this.parameters), 'object', 'Missing transform parameters');
      var xfinal, yfinal;
      ...
      var zmeter = unit(zraw, zunit).to('m').toNumeric(); // Apply the geoid height (to get the ellipsoid/orthometric height)
      ...
      var zfinal = unit(zfinalm, 'm').to(outputZUnit).toNumeric();
      return {
    ...
    value: function _applyMatrixToPoint(x, y, z) {
      strict.equal(_typeof(this.matrix), 'object', 'Missing transform matrix');
      ...
      if (inputXYUnit === outputXYUnit) {
        xfinal = unit(xraw, outputXYUnit).toNumeric();
        yfinal = unit(yraw, outputXYUnit).toNumeric();
        zfinal = unit(zraw, zunit).to(outputZUnit).toNumeric();
      } else if (inputXYUnit === 'dd' && outputXYUnit !== 'dd') {
        // Convert from WGS84 -> LOCAL
        // 1. z value should be in meters (and assumed to be already height above ellipsoid)
        var zmeter = unit(zraw, zunit).to('m').toNumeric();
        var lon = xraw;
        ...
        zfinal = unit(_zmeter, 'm').to(outputZUnit).toNumeric();
      }
